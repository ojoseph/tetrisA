...
public class createField : MonoBehaviour {
	...
	void makeField(){
		...
		int theNumRows = (theField.Length/10); 
			...
			for(int incre = 1; incre < 2; incre++){
				...
				//	print("RESELECT");
					...
					while(slctRndCube == slctPrevRndCube){
						//print ("We reselect");
						slctRndCube = Random.Range(1, cubeType.Length);
					}	
		...
		for(int theRow = 0 ; theRow < theNumRows; theRow++){
			...
			for(int incre = 0; incre < 11; incre++){
				...
				if(theField[theRow,incre] != 0){
					...
					if(theField[theRow,incre] == 1){
							
					//We get the location of where we want to put our token
						GameObject targetLoc = new GameObject();
						targetLoc = GameObject.Find(theTileNames[theRow,incre]);
						
						aCube = Instantiate( Resources.Load("blocks/heart"),  new Vector3(targetLoc.transform.position.x, 0, targetLoc.transform.position.z)/*transform.localPosition*incre*/, transform.localRotation) as GameObject;
						aCube.name = theTileNames[ theRow,incre ];
